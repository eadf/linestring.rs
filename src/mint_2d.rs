    ...
    T: num_traits::Float + std::fmt::Debug + approx::AbsDiffEq + approx::UlpsEq + Sync,
{
    ...
    pub fn copy_to_3d(&self, plane: mint_3d::Plane) -> mint_3d::Line3<T> {
        match plane {
            mint_3d::Plane::XY => mint_3d::Line3::<T>::from([
                self.start.x,
                self.start.y,
                T::zero(),
                self.end.x,
            ...
            mint_3d::Plane::XZ => mint_3d::Line3::<T>::from([
                self.start.x,
                T::zero(),
                self.start.y,
                self.end.x,
            ...
            mint_3d::Plane::ZY => mint_3d::Line3::<T>::from([
                T::zero(),
                self.start.x,
                self.start.y,
                T::zero(),
    ...
    T: num_traits::Float + std::fmt::Debug + approx::AbsDiffEq + approx::UlpsEq + Sync,
{
    ...
    pub fn discretise_3d_straight_line(&self) -> mint_3d::LineString3<T> {
    ...
    T: num_traits::Float + std::fmt::Debug + approx::AbsDiffEq + approx::UlpsEq + Sync,
{
    ...
    #[inline(always)]
    pub fn contains_point_inclusive(&self, point: &mint::Point2<T>) -> bool {
        if let Some(self_aabb) = self.min_max {
    ...
    fn contains_point_inclusive_(
        aabb: &(mint::Point2<T>, mint::Point2<T>),
        point: &mint::Point2<T>,
    ) -> bool {
...
impl<T: num_traits::Float + std::fmt::Debug + approx::AbsDiffEq + approx::UlpsEq + Sync> Default
    for SimpleAffine<T>
{
    #[inline]
    fn default() -> Self {
        ...
        + num_traits::cast::NumCast,
{
                ...
                if let Some(destination_low) = b_aabb.get_low() {
                    if let Some(destination_high) = b_aabb.get_high() {
                        ...
                        let source_aabb_size = [
                            (source_high.x - source_low.x).max(min_dim),
                            (source_high.y - source_low.y).max(min_dim),
                        ];
                        ...
                        let dest_aabb_size = [
                            (destination_high.x - destination_low.x).max(min_dim),
                            (destination_high.y - destination_low.y).max(min_dim),
                        ];
    ...
    /// let mut aabb_source = mint_2d::Aabb2::<T>::from([-100.,-100.,100.,100.]);
    /// // dest is (0,0)-(800,800.)
    /// let mut aabb_dest = mint_2d::Aabb2::<T>::from([0.,0.,800.,800.]);
    ...
    /// type T = f32;
    /// // source is (0,0)-(1,1)
    /// let mut aabb_source = mint_2d::Aabb2::<T>::from([0.,0.,1.,1.]);
    /// // dest is (1,1)-(2,2)
    /// let mut aabb_dest = mint_2d::Aabb2::<T>::from([1.,1.,2.,2.]);
    ///
    /// let transform = mint_2d::SimpleAffine::new(&aabb_source, &aabb_dest).unwrap();
    /// assert_eq!(
