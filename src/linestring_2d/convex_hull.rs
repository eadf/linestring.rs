...
use cgmath::{ulps_eq, MetricSpace, Point2};
use rayon::prelude::*;
...
impl<T: cgmath::BaseFloat + Sync> ConvexHull<T> {
    ...
    fn find_lowest_x(linestring: &[Point2<T>]) -> (usize, Point2<T>) {
    ...
    /// let b = Point2 { x: 0.0, y: 10.0 };
    /// let c = Point2 { x: -10.0, y: 5.0 };
    ...
    ///```
    #[inline(always)]
    pub fn is_point_left(a: Point2<T>, b: Point2<T>, c: Point2<T>) -> bool {
    ...
    ///```
    #[inline(always)]
    pub fn is_point_left_allow_collinear(a: Point2<T>, b: Point2<T>, c: Point2<T>) -> bool {
    ...
    ///```
    /// # use linestring::linestring_2d::convex_hull;
    /// # use cgmath::{AbsDiffEq,UlpsEq,Point2};
    /// let a = Point2 { x: 0.0_f32, y: 0.0 };
    /// let b = Point2 { x: 0.0_f32, y: 10.0 };
    ...
    ///```
    #[inline(always)]
    pub fn cross_2d(a: Point2<T>, b: Point2<T>, c: Point2<T>) -> T {
        (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x)
    }
    ...
    /// let center = Point2{x:2000_f32, y:2000.0};
    ///
    ...
    where
        I: IntoIterator<Item = &'a Point2<T>>,
        T: 'a,
    {
        let input_points = input.into_iter().copied().collect::<Vec<Point2<T>>>();
    ...
    where
        ...
        T: 'a + Sync,
    {
        let mut input_points = input.into_iter().copied().collect::<Vec<Point2<T>>>();
        ...
        let comparator = |b: &Point2<T>, c: &Point2<T>| {
            ...
            match T::zero().partial_cmp(&ccr) {
                ...
                Some(Ordering::Equal) => {
                    let dist_sb = starting_point.distance2(*b);
                    let dist_sc = starting_point.distance2(*c);
                    if dist_sb > dist_sc {
                        Ordering::Greater
                    } else {
                        Ordering::Less
    ...
    ///
    ///```
    pub fn contains_point_exclusive(a: &linestring_2d::LineString2<T>, p: &Point2<T>) -> bool {
        if a.len() <= 1 {
            return false;
        }
        a.as_lines()
    ...
    /// assert!(convex_hull::ConvexHull::contains_point_inclusive(&hull, [10.0,5.0].into()));
    ///```
    pub fn contains_point_inclusive(a: &linestring_2d::LineString2<T>, p: Point2<T>) -> bool {
        if a.len() <= 1 {
            return false;
        }
        a.as_lines()
