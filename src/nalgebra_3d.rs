...
impl Plane {
    ...
    #[allow(dead_code)]
    #[inline(always)]
    fn to_2d<T>(&self, point: &nalgebra::Point3<T>) -> nalgebra::Point2<T>
    where
        T: nalgebra::RealField + Sync + approx::AbsDiffEq<Epsilon = T>,
    {
        match self {
            Plane::XY => nalgebra::Point2::new(point.x, point.y),
            Plane::XZ => nalgebra::Point2::new(point.x, point.z),
            Plane::ZY => nalgebra::Point2::new(point.z, point.y),
        }
    }
}
    ...
    T: nalgebra::RealField + Sync,
{
    ...
    pub fn triangle_area_squared_times_4(
        p1: &nalgebra::Point3<T>,
        p2: &nalgebra::Point3<T>,
        p3: &nalgebra::Point3<T>,
    ) -> T {
        let v1_x = p1.x - p2.x;
        let v1_y = p1.y - p2.y;
        let v1_z = p1.z - p2.z;
    ...
    T: nalgebra::RealField + Sync,
{
    ...
    pub fn with_points(mut self, points: Vec<nalgebra::Point3<T>>) -> Self {
        self.points = points;
        self
    }
    ...
    /// An axis will always try to keep it's position (e.g. y goes to y if possible).
    /// That way the operation is reversible (with regards to axis positions).
    pub fn copy_to_2d(&self, plane: Plane) -> nalgebra_2d::LineString2<T> {
        let mut rv: nalgebra_2d::LineString2<T> = match plane {
            Plane::XY => self
                .points
                .iter()
    ...
    pub fn simplify(&self, distance_predicate: T) -> Self {
        ...
        if self.connected {
            ...
            let mut rv: Vec<nalgebra::Point3<T>> = Vec::with_capacity(points.len());
            ...
            let mut rv: Vec<nalgebra::Point3<T>> = Vec::with_capacity(self.points.len());
            // _simplify() always omits the the first point of the result, so we have to add that
    ...
    fn _simplify(
        ...
        slice: &[nalgebra::Point3<T>],
    ) -> Vec<nalgebra::Point3<T>> {
    ...
    T: nalgebra::RealField + Sync,
{
    ...
    /// An axis will always try to keep it's position (e.g. y goes to y if possible).
    /// That way the operation is reversible (with regards to axis positions).
    pub fn copy_to_2d(&self, plane: Plane) -> nalgebra_2d::LineStringSet2<T> {
        let mut rv = nalgebra_2d::LineStringSet2::with_capacity(self.set.len());
    ...
    T: nalgebra::RealField + Sync,
{
    ...
    #[inline(always)]
    pub fn contains_point(&self, point: &nalgebra::Point3<T>) -> bool {
        if let Some(self_aabb) = self.min_max {
