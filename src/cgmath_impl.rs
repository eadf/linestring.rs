...
where
    T: Copy + fmt::Debug + cmp::PartialOrd,
{
    aabb_min: Option<cgmath::Point2<T>>,
    aabb_max: Option<cgmath::Point2<T>>,
}
...
where
    T: Copy + fmt::Debug + cmp::PartialOrd,
{
    aabb_min: Option<cgmath::Point3<T>>,
    aabb_max: Option<cgmath::Point3<T>>,
}
    ...
    T: Copy + fmt::Debug + cmp::PartialOrd,
{
    ...
    pub fn update(&mut self, point: &cgmath::Point2<T>) {
    ...
    T: Copy + fmt::Debug + cmp::PartialOrd,
{
    ...
    pub fn update(&mut self, point: &cgmath::Point3<T>) {
        if self.aabb_max.is_none() {
            self.aabb_max = Some(*point);
        }
        if self.aabb_min.is_none() {
