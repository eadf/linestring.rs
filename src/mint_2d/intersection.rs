...
use crate::{mint_2d, LinestringError};
    ...
    T: num_traits::Float + std::fmt::Debug + approx::AbsDiffEq + approx::UlpsEq,
{
    fn partial_cmp(&self, other: &Self) -> Option<cmp::Ordering> {
        if mint_2d::ulps_eq(&self.pos.y, &other.pos.y) {
            if mint_2d::ulps_eq(&self.pos.x, &other.pos.x) {
    ...
    fn update(&mut self, pivot_x: T, candidate_x: T, candidate_slope: T, candidate_index: usize) {
        if candidate_x < pivot_x {
            ...
            if let Some(current_min) = self.best_left {
                if mint_2d::ulps_eq(&current_min, &candidate_x) {
                    self.slope
            ...
            if let Some(current_max) = self.best_right {
                if mint_2d::ulps_eq(&current_max, &candidate_x) {
                    self.slope
    ...
    T: num_traits::Float + std::fmt::Debug + approx::AbsDiffEq + approx::UlpsEq,
{
    ...
    fn update_both(&mut self, candidate_index: usize, lines: &[mint_2d::Line2<T>]) {
        let line = lines[candidate_index];
        let candidate_slope = if mint_2d::ulps_eq(&line.end.y, &line.start.y) {
            T::infinity()
        } else {
    ...
    fn update_left(&mut self, clear: bool, candidate_slope: T, candidate_index: usize) {
        ...
        if let Some(current_slope) = self.best_left {
            if mint_2d::ulps_eq(&current_slope, &candidate_slope) {
    ...
    T: num_traits::Float + std::fmt::Debug + approx::AbsDiffEq + approx::UlpsEq,
{
    ...
    pub fn get_lines(&self) -> &Vec<mint_2d::Line2<T>> {
        &self.lines
    }
    ...
    where
        I: Iterator<Item = mint_2d::Line2<T>>,
    {
    ...
    where
        T: 'a,
        I: Iterator<Item = &'a mint_2d::Line2<T>>,
    {
        let mut site_events = self.site_events.take().unwrap();
    ...
    fn add_intersection_event(
        ...
        if let Some(event) = site_events.get_mut(position) {
            ...
            for new_intersection in intersecting_lines.iter() {
                ...
                if mint_2d::point_ulps_eq(&position.pos, &i_line.start)
                    || mint_2d::point_ulps_eq(&position.pos, &i_line.end)
                {
                    continue;
                }
    ...
    pub fn compute(
        ...
        loop {
            if let Some((key, event)) = site_events.pop_pair() {
                ...
                self.sweepline_pos = mint::Point2 {
                    x: T::max_value(),
                    y: T::max_value(),
                };
    ...
    fn find_new_events(
        ...
        for left_i in left.iter() {
            for right_i in right.iter() {
                ...
                if mint_2d::point_ulps_eq(&left_l.end, &right_l.end) {
                ...
                if let Some(intersection_p) = left_l.intersection_point(&right_l) {
                    ...
                    if intersection_p.y >= self.sweepline_pos.y
                        ...
                        && !mint_2d::point_ulps_eq(&intersection_p, &self.sweepline_pos)
                    {
                        #[cfg(feature = "console_trace")]
                        println!(
