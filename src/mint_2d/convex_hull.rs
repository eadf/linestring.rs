        ...
        + approx::AbsDiffEq<Epsilon = T>,
{
    ...
    /// }
    ///```
    pub fn gift_wrap<'a, I>(input: I) -> mint_2d::LineString2<T>
    where
        I: IntoIterator<Item = &'a mint::Point2<T>>,
        T: 'a,
    {
        let input_points = input.into_iter().copied().collect::<Vec<mint::Point2<T>>>();

        if input_points.len() <= 3 {
            //println!("shortcut points {:?} connected:{}", linestring.len(), linestring.connected);
            let rv = mint_2d::LineString2::<T>::with_capacity(input_points.len())
                .with_connected(true)
        ...
        let mut rv =
            mint_2d::LineString2::<T>::with_capacity(input_points.len()).with_connected(true);
